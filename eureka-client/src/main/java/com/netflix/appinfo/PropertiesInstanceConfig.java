 ...
 * </p>
 *在configuration中提供了注册eureka server所需的信息
 * 配置文件。在类路径中搜索配置文件
 * 使用属性<em>eureka.client指定的名称。道具< / em >和
 * 后缀< em > . properties < / em >。如果没有指定属性，
 * < em > eureka-client。属性</em>是默认值。的属性
 * 使用传递给该类的<em>名称空间</em>来查找。
 * <p>
 ...
 * </p>
 * 如果< em >尤里卡。另外，还指定了environment</em>属性
 * < em > eureka-client——< eureka.environment >。属性</em>是另外加载的
...
public abstract class PropertiesInstanceConfig extends AbstractInstanceConfig implements EurekaInstanceConfig {
    ...
    public PropertiesInstanceConfig(String namespace, DataCenterInfo info) {
        ...
        appGrpNameFromEnv = ConfigurationManager.getConfigInstance()
                //FALLBACK_APP_GROUP_KEY: NETFLIX_APP_GROUP
                .getString(FALLBACK_APP_GROUP_KEY, Values.UNKNOWN_APPLICATION);
        //eureka-client // 初始化 配置文件对象
