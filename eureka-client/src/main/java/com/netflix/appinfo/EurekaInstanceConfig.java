...
public interface EurekaInstanceConfig {
     ...
     * 获取要在eureka中注册的此实例的惟一Id(在appName的范围内)。
     ...
     * traffic.
     * 获取实例应该接收的<code>non-secure</code>端口
     * 流量。
     ...
     * instance.
     *指示eureka客户机需要发送的频率(以秒为单位)
     * 心跳到eureka服务器，表明它仍然活着。如果
     * 在指定的时间段内不接收心跳
     * {@link #getLeaseExpirationDurationInSeconds()}， eureka服务器将删除
     * 实例从它的视图中，通过不允许流量到这里
     * 实例。
     * <p>
     ...
     * 指示eureka服务器此后等待的时间(以秒为单位)
     * 在从它的心跳中删除此实例之前接收到的最后一个心跳
     * 通过禁用到此实例的流量来查看。
     * <p>
     ...
     * .
     * 设置此值太长可能意味着流量可能被路由
     * 即使实例不是活动的，也要对该实例执行。设置这个值
     * 太小可能意味着，实例可能会因为
     * 临时网络故障。此值至少要设置为大于
     * 在 {@link #getLeaseRenewalIntervalInSeconds()}
     * </p>
     * 租约过期时间，单位：秒
